(function () {
	...
	function renderSuggestions(q) {
		...
		const matches = state.videos.filter((v) => v.title.toLowerCase().includes(q.toLowerCase())).slice(0, 5);
	...
	function openModal(video) {
		...
		const embedUrl =
			'https://www.youtube.com/embed/' + video.id + '?autoplay=1&rel=0&modestbranding=1&enablejsapi=1&start=' + Math.floor(startAt);
		...
		el.watchLaterContainer.innerHTML = state.watchLater
			.map((v) => {
				...
				return (
					...
					'" loading="lazy">' +
					'<button class="watch-later-btn active" data-id="' +
					v.id +
					...
					utils.highlight(utils.sanitize(utils.truncate(v.title, 60)), state.search) +
					'</div>' +
					'<div class="card-meta">' +
					'<span class="card-tag">' +
					...
					'</span>' +
					'<span>' +
					utils.formatDate(v.publishedAt) +
					'</span>' +
					'</div></div></div>'
				);
			})
	...
	function renderDashboard() {
		...
		if (el.dashCategories) {
			...
			el.dashCategories.innerHTML =
				Object.entries(counts)
					.sort((a, b) => b[1] - a[1])
					.map(
						([key, count]) =>
							'<div class="dashboard-list-row">' + '<span>' + categoryLabel(key) + '</span>' + '<strong>' + count + '</strong>' + '</div>',
					)
				...
				? state.watchLater
						...
						.map(
							(v) =>
								...
								'<span>' +
								utils.sanitize(utils.truncate(v.title, 40)) +
								'</span>' +
								'<strong>' +
								categoryLabel(v.category) +
								'</strong>' +
	...
	function card(v) {
		...
		return (
			...
			(getProgress(v.id)
				? '<div class="progress-bar-container"><div class="progress-bar-fill" style="width:' + getProgress(v.id).percent + '%"></div></div>'
				: '') +
